...
- # 一、概念题
  collapsed:: true
	- ## 1、简单聊一聊Binder机制？
	  collapsed:: true
		- 1、Binder机制是一个基于Binder驱动和内核的机制，用于Android进程间通信机制。
		- 2、基于Client / Server架构的，服务端创建Binder对象，客户端创建Binder代理对象，通过底层binder驱动来间接调用服务端方法
...
- # ServiceManager相关
  collapsed:: true
	- ## 1、ServiceManager介绍 / Binder机制中如何找到目标Binder的
		...
		- 1、我们无法直接获取 AMS，PMS等Ibinder对象进行进程间通信
	...
	- ## 3、[[SystemServiceManager和ServiceManager区别]]
- # 参考

...
- # 11、方法区中的类，会在哪个时候卸载，回收？
  collapsed:: true
	...
	- 3个条件同时满足，可被回收，但是可被禁用
		- ![image.png](../assets/image_1689481955659_0.png)
...
- # 19、在分代模式，为什么新生代，要分为Eden区from ，to，区。比例为8:1:1。
  collapsed:: true
	...
	- from 和 to 是复制回收算法，from区满了，把存活的对象复制到to区，对象年龄+1（age存放在对象头中）把from区清空。反之亦然。这里年龄最大15，达到之后进入老年代
- # 19、[[垃圾回收算法]]
- # 20、JVM常量池都有哪几种，以及区别？
  collapsed:: true
	- 详见[[JVM常量池梳理]]
...
- # 23、java对象会不会分配在栈中？
	- 栈上分配
	- [[对象分配策略-面试]]
- # 24、如果判断一个对象是否被回收，由哪些算法，实际虚拟机用的最多的是什么？
	- [[判断对象的存活算法]]
	- 答出 引用计数法，缺点相互引用的问题
	- 实际虚拟机用的最多的是可达性分析法
...
- # 26、JVM中一次完整的GC流程是怎样的？对象如何晋级到老年代？
	...
	- 对象进入From 或者 to 区，空间不够了再进行一次minor GC 或者 Young Gc, 再进入to区或者from区。直到1、年龄达到15,2 超过Server区一半，的对象年龄是一样的。则进入老年代
	- ## 如何进入老年代？
		- 1、大对象直接进入（长的字符串，数组）

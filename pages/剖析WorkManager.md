...
- 利用contentProvider来初始化SDK，这样可以做到SDK代码与平台完全隔离。也不用担心业务方忘记初始化或者初始化时序不对的问题。
  利用数据库+AlarManager的思路来处理app异常退出重启任务策略。比如说我们可以记录下崩在哪个页面了，下次进入在自动打开那个页面。当然也分场景，如果那个页面必现崩溃，然后再这么搞就完犊子了。
  利用PackageManagerHelper.setComponentEnabled(）来关闭一些不用的广播或者服务，等真正使用的时候在开启，而不用反复注册、解注册。这样可以可以优化一部分app性能。
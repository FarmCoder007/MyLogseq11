- # 一、享元模式定义（通常配合工厂模式）
	- 运用共享技术来有效地支持[[#red]]==**大量细粒度对象的复用，内存复用**==。它通过共享已经存在的对象来大幅度减少需要创建的对象数量、避免大量相似对象的开销，从而提高系统资源的利用率。
- # 二、享元模式结构
  collapsed:: true
...
- # 三、享元模式Java实例
  collapsed:: true
	- 下面的图片是众所周知的俄罗斯方块中的一个个方块，如果在俄罗斯方块这个游戏中，每个不同的方块都是一个实例对象，这些对象就要占用很多的内存空间，下面利用享元模式进行实现。
	  collapsed:: true
		- ![俄罗斯方块种类](https://www.panziye.com/wp-content/uploads/2022/06/2022060406555277.jpeg)
	- 先来看采用享元模式设计的类图：
	  collapsed:: true
		- ![享元模式类图](https://www.panziye.com/wp-content/uploads/2022/06/2022060406565745.png)
	- ### 代码如下：
	  collapsed:: true
		- ### 抽象享元角色
		  collapsed:: true
		...
		- ### 测试类
		  collapsed:: true
			- ```java
			  ...
			  ```
		- 运行测试类，结果如下：
			- ![享元模式模式案例运行结果](https://www.panziye.com/wp-content/uploads/2022/06/2022060407043924.png)
- # 四、享元模式优缺点
	- ### 1，优点
...
- # 六、JDK源码应用享元模式解析
	- `Integer`类使用了享元模式。我们先看下面的例子：
	  collapsed:: true
		...
		- 运行上面代码，结果如下：
		  collapsed:: true
			- ![JDK源码应用案例结果](https://www.panziye.com/wp-content/uploads/2022/06/202206040708126.png)
